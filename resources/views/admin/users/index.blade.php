...
/* optional: biar posisinya tetap di kanan */
...
.page-cta{
  ...
  margin-top:-10px;    /* geser agak ke atas */
}
:root{
...
.btn-cta__icon{ display:grid; place-items:center; width:18px; height:18px; }
.btn-cta__icon svg{ width:18px; height:18px; stroke-width:3; }
...
.modal__panel{
  position:relative; margin:60px auto; max-width:780px;
  background:#f1faf1; border:1px solid var(--bd); border-radius:16px;
...
.modal__close{
  background:#fff; border:1px solid var(--bd); width:36px; height:36px;
...
.form-input,.form-select{
  width:100%; border:1px solid var(--bd); background:#fff;
...
.page-header {
  ...
  align-items: flex-start; /* biar sejajar ke atas */
  margin-bottom: 1rem;
}

.page-header h2 {
  margin: 0; /* rapihin biar nggak dorong ke bawah */
}
    ...
    <tbody>
      @forelse ($users as $u)
        ...
        <tr>
          ...
          <td>{{ $u->username }}</td> {{-- ✅ tampilkan username --}}
          <td>{{ $u->email }}</td>
          <td>
            @if($roleLc === 'admin')
              ...
              <td>
                <div class="actions">
                  ...
                  <form action="{{ route('admin.users.updateRole', $u) }}" method="POST">
                    @csrf
                    @method('PATCH')
                    <select name="role" class="sel" style="padding:6px 8px;font-size:13px">
                      <option value="admin" @selected($roleLc==='admin')>Admin</option>
                      <option value="user"  @selected($roleLc==='user')>User</option>
                    </select>
                    <button class="icon-chip" title="Simpan role">💾</button>
                  </form>
...
{{-- ===== Modal: Create User (tanpa Jabatan) ===== --}}
<div id="createModal" class="modal" aria-hidden="true" role="dialog" aria-modal="true" aria-labelledby="createTitle">
  ...
  <div class="modal__panel">
    ...
    <form method="POST" action="{{ route('admin.users.store') }}">
      @csrf
      {{-- penanda agar modal auto-kebuka saat validasi gagal --}}
...
(function(){
  ...
  // Auto-open jika validasi create gagal
